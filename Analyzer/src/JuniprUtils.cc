...
#include <cmath>
#include <algorithm>
#include "../include/JuniprUtils.hh"
...
double get_theta(vector<double> v1, vector<double> v2){
  ...
  //std::cout << " v1_norm " << v1_norm  << " v2_norm " << v2_norm << std::endl;

  if(abs(cosTheta-1)<1e-6){
    return 0;
  }else if(abs(cosTheta+1)<1e-6){
    return M_PI;
  }else if(abs(cosTheta-1)>1e6){
    return 0;
  }else if(abs(cosTheta)!=abs(cosTheta)){
    return 0;
  }else{
...
vector<double> PJ_to_ETPM(PseudoJet PJ, PseudoJet ref){
  ...
  if((E==0. && mass==0.) || (E<1e-6 && mass<1e-6)) {
    theta = 0; phi=0;
  }
  ...
  //std::cout << "e " << E << " theta " << theta << " phi " << phi << " mass " << mass << std::endl;
