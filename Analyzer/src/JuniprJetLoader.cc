...
JuniprJetLoader::JuniprJetLoader(ClusterSequence cs, int label){
  cluster_sequence_to_JuniprJet(cs,label);
}

JuniprJetLoader::~JuniprJetLoader(){
  delete juniprjet;
}

vector<double> JuniprJetLoader::get_branching(vector<PseudoJet> const &PJ_CSJets, int hist_id, vector<int> current_daughters){
...
void JuniprJetLoader::remove_hist_id(vector<int> &current_intermediate_state, int hist_id){
...
void JuniprJetLoader::add_hist_id(vector<int> &current_intermediate_state, int hist_id, vector< vector<double> > const &CSJets){
    ...
    for (int i = 0; i<int(current_intermediate_state.size()); i++){
...
void JuniprJetLoader::cluster_sequence_to_JuniprJet(ClusterSequence cs, int label){
    ...
    PseudoJet jet_axis = PJ_CSJets[int(PJ_CSJets.size()-1)];  

    //std::cout << " ref " << jet_axis.E() << jet_axis.eta() << std::endl;

    // Convert CSJets from PseudoJet to format (E, theta, phi, mass)
    vector< vector<double> > CSJets;
    for (int i = 0; i<int(PJ_CSJets.size()); i++){
    ...
    current_intermediate_state.push_back(int(hist.size()-2));// Add seed particle
    ...
    for(int hist_id = int(hist.size()-2); hist_id>=0; hist_id--){
    ...
    juniprjet = new JuniprJet(CSJets,
			      intermediate_states,
			      mothers,
			      daughters,
			      branchings,
...
void JuniprJetLoader::write_juniprjet(ostream& outfile, vector < double > vars){
  juniprjet->write_to_json(outfile,vars);
}
